...
- 诊断用户当前预览中已经出现的报错时，先调用 get_preview_errors，mode 使用 current，以保留用户的页面和交互现场；修改文件后需要验证结果时，再使用 refresh 模式加载最新工作区并检查新报错。
...
export const CONVERSATION_TITLE_PROMPT =
  '根据用户的第一条消息生成简短会话标题，仅输出标题。必须使用用户消息的主要语言，不得翻译成其他语言：英文消息必须生成英文标题，中文消息必须生成中文标题。中文标题不超过 16 个汉字，英文标题不超过 8 个单词，其他语言保持同等简洁。'
